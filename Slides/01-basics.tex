...
\codechellatitle
  ...
  {Day 1: The 2$\times$2, Parallel Trends, and Event Studies}
...
\vspace{0.4em}
\footnotesize
Scale the example up: \highlight{Texas, 254 counties, 30M people.} 5 biggest counties hold roughly half the population. Heterogeneous $\delta_i$ across people, with the high-$\delta$ folks concentrated in the 5 big counties and the low-$\delta$ folks in the 249 small ones.
\vspace{0.5em}
\begin{itemize}
  \item \textbf{Per-person ATE}: $\;\tfrac{1}{N}\sum_i \delta_i$ --- weight every person by $1/N$. \;\highlight{Answer: $+2$.}
  \item \textbf{Avg of county ATEs}: $\tfrac{1}{254}\sum_c \overline{\delta}_c$ --- weight every county by $1/254$. \;\highlight{Answer: $-0.88$.}
\end{itemize}
\vspace{0.5em}
\begin{center}
...
%% --- 8.3c The weighted-mean formula ---
\begin{frame}[fragile]{The weighted mean is a covariance correction}
\vspace{0.4em}
For any outcome $Y$ and any weight $S$:
\vspace{-0.2em}
\[
\widetilde{\mathbb{E}}[Y] \;\equiv\; \frac{\mathbb{E}[S\,Y]}{\mathbb{E}[S]}
\;\;=\;\; \mathbb{E}[Y] \;+\; \dfrac{\mathrm{Cov}(S,\,Y)}{\mathbb{E}[S]}
\]
\vspace{-0.4em}
\begin{itemize}\setlength\itemsep{2pt}\footnotesize
  \item Weighted mean $=$ unweighted mean $+$ a \highlight{covariance correction}.
  \item $\mathrm{Cov}(S, Y) > 0 \Rightarrow$ weighted exceeds unweighted; $< 0 \Rightarrow$ smaller.
  \item Ten-people table: weight by county size $S \in \{8, 2\}$ and compute $\widetilde{\mathbb{E}}[\delta]$.
\end{itemize}
\vspace{0.5em}
...
gen size = 8 if county==1
replace size = 2 if county==2
summarize delta [aweight=size]
* weighted mean = +0.2
summarize delta
...
df$size <- ifelse(df$county==1, 8, 2)
weighted.mean(df$delta, df$size)
# weighted mean = +0.2
mean(df$delta)
# unweighted mean = -0.25
\end{lstlisting}
...
di (1.802504 - 1.775398) ///
 - (1.254353 - 1.282884)
\end{lstlisting}
\begin{center}
\footnotesize $\hat\beta_{2004} =$ \highlight{$\mathbf{0.0556}$}
\end{center}
\column{0.50\textwidth}
...
\end{center}
\meta{Cluster-robust SE: 0.060. \;Match to 4 decimals.}
\end{columns}
\vspace{0.3em}
\begin{center}
...
\begin{center}
\includegraphics[height=0.54\textheight,keepaspectratio]{figures/Miller_Medicaid4.png}
\end{center}
\vspace{-0.1em}
\footnotesize
\begin{itemize}\setlength\itemsep{2pt}
