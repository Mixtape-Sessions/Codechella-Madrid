%% 02-covariates.tex — Codechella Madrid Day 2: Covariates in DiD
...
\codechellatitle
  ...
  {CUNEF Auditorium \textperiodcentered\ May 25--28, 2026}

%% ---------- Slide 2: Your DiD lives or dies by PT ----------
\begin{frame}{Your DiD lives or dies by parallel trends}
\vspace{0.5em}
With \textbf{no anticipation} and a \textbf{clean (untreated) comparison group}, the 2$\times$2 is always and only:
\vspace{0.3em}
\begin{center}
\Large
$\widehat{\text{2}\times\text{2}} \;=\; \text{ATT} \;+\; \text{non-PT bias}$
\end{center}
\vspace{0.4em}
\begin{itemize}
  \item PT is the \highlight{only} condition you need for unbiasedness.
  \item The 2$\times$2 \textit{is whatever it is}. No estimation trick saves it. If PT holds, the number you computed yesterday \textit{is} the ATT.
  \item So your paper's evidence is mostly about \highlight{defending PT} --- not about estimation.
\end{itemize}
\end{frame}

%% ---------- Slide 3: What if PT fails? ----------
\begin{frame}{What if PT fails?}
\vspace{0.5em}
Then to keep using DiD, you have to \textit{recover} it. Two options:
\vspace{0.5em}
\begin{enumerate}\setlength\itemsep{0.5em}
  \item \highlight{Triple differences.} A second between-group difference absorbs the differential trend. \\
        \textit{Harder design. Recent audits suggest most papers are doing it incorrectly.}
  \item \highlight{Covariates.} Condition on $X$ so that PT holds \textit{within strata of $X$}, even if it fails unconditionally.
\end{enumerate}
\vspace{0.7em}
\meta{Today: option 2. \;Covariates in DiD serve a non-intuitive purpose --- they're not for ``controlling.'' \;They're for fixing broken parallel trends.}
\end{frame}

%% ---------- Slide 4: Covariates as the fix ----------
\begin{frame}{Covariates as the fix for broken PT}
\vspace{0.5em}
We'll cover a variety of estimators that incorporate covariates into the DiD estimation:
\vspace{0.4em}
\begin{itemize}\setlength\itemsep{0.4em}
  \item \highlight{OR} --- outcome regression (Heckman, Ichimura \& Todd 1997)
  \item \highlight{IPW} --- inverse propensity weighting (Abadie 2005)
  \item \highlight{DR-DiD} --- doubly robust (Sant'Anna \& Zhao 2020)
\end{itemize}
\vspace{0.5em}
We'll contrast each with the workhorse: \highlight{OLS TWFE with controls}.
\vspace{0.3em}
\meta{Spoiler: TWFE-with-controls is the false friend. It looks like it should work. It doesn't.}
\end{frame}
...
\begin{center}
\footnotesize
$\underbrace{\;\;\delta \text{ doesn't depend on } X\;\;}_{\textstyle\color{accent}\textbf{Bias}_{\text{HTE}}}
\;\;\;
\underbrace{\;\;X \text{ has parallel trends across groups}\;\;}_{\textstyle\color{accent}\textbf{Bias}_{\Delta X}}
\;\;\;
\underbrace{\;\;X \text{ is time-invariant}\;\;}_{\textstyle\color{accent}\textbf{Bias}_{\text{FE}}}$
\end{center}

\vspace{0.4em}
\begin{itemize}\setlength\itemsep{1pt}\footnotesize
  \item $\textbf{Bias}_{\textbf{HTE}}$. \;If $\delta(X)$ varies with $X$, TWFE fits one $\delta$. \;\textit{Y(1) problem. \;Sign indeterminate.}
  \item $\textbf{Bias}_{\boldsymbol{\Delta X}}$. \;If $X$ is imbalanced and drives $Y(0)$'s trend, PTA fails. \;\textit{Y(0) problem. \;Sign indeterminate.}
  \item $\textbf{Bias}_{\textbf{FE}}$. \;$\alpha_i$ demeans time-invariant $X$ to zero, silently restoring \textit{unconditional} PT. \;Fix: $\text{Post}\times X_{i,0}$. \;\textit{Sign indeterminate.}
\end{itemize}
\end{frame}

%% ===========================================================
%%   Bias_HTE — derived via Scott's framework: TWFE makes Y(0) and Y(1) share
%%   the same slope on X; if true slopes differ, ATT = delta + (theta_1 - theta_2) X.
%% ===========================================================
...
\begin{frame}[fragile]{Build $Y(0)$ and $Y(1)$ by hand --- homogeneous vs.\ heterogeneous in $X$}
\vspace{0.2em}
\footnotesize
Same $Y(0)$ in both. \;The treatment effect $Y(1) - Y(0)$ is either a \textit{constant} or a \textit{function of $X$}.
\vspace{0.4em}
\begin{columns}[T,onlytextwidth]
\column{0.48\textwidth}
\textbf{Homogeneous TE in $X$ \;(Assumption 4 holds)}
...
\column{0.48\textwidth}
\textbf{Heterogeneous TE in $X$ \;(Assumption 4 fails)}
\begin{lstlisting}[basicstyle=\ttfamily\tiny]
gen y0 = 15000 + 10.25*age - 10.5*age_sq ///
              + 1000*gpa - 10.5*gpa_sq   ///
              + 500*age*gpa + e
...
\begin{itemize}\setlength\itemsep{1pt}
  \item \textbf{Left:} $Y(0)$ and $Y(1)$ share \textit{the same} slope on $X$. \;ATT $= 2{,}500$ for everyone. TWFE-with-$X$ recovers it.
  \item \textbf{Right:} $Y(1)$ slopes differ. \;A 1-year-older worker gains \$100 more; a 1-GPA-point higher worker gains \$1{,}100 more. \;\highlight{TWFE fits one $\delta$, misses the variation.}
\end{itemize}
\end{frame}
...
%% ---------- L2-DX-1a: RCT analogy ----------
\begin{frame}{$\text{Bias}_{\Delta X}$: the RCT analogy}
\vspace{0.5em}
Earnings $Y$ depends on a million $X$s: \;gender, age, education, neighborhood, race, occupation, parental income, $\ldots$

\vspace{1.2em}
Randomization \textit{balances} those $X$s across treated and control.

\vspace{0.8em}
\begin{center}
\highlight{The $X \to Y$ channels cancel.}
\end{center}
\end{frame}

%% ---------- L2-DX-1b: DiD without randomization ----------
\begin{frame}{DiD doesn't get balance for free}
\vspace{0.6em}
...
\vspace{1.2em}
\begin{center}
\highlight{And PTA on $Y(0)$ fails.}
\end{center}
\end{frame}

%% ---------- L2-DX-1c: Gender setup ----------
\begin{frame}{A simple example: gender}
\vspace{0.4em}
...
\vspace{1em}
\highlight{No treatment effect anywhere.} \;Earnings just drift up faster for men than for women.

\vspace{1em}
Composition: \;\textbf{75\% male in treated, 25\% male in control.}
\end{frame}

%% ---------- L2-DX-1d: The arithmetic ----------
\begin{frame}{Naive DiD reads the gap}
\vspace{0.6em}
\begin{align*}
\mathbb{E}[\Delta Y(0) \mid D{=}1] &\;=\; 0.75(2) + 0.25(1) \;=\; 1.75 \\[0.4em]
\mathbb{E}[\Delta Y(0) \mid D{=}0] &\;=\; 0.25(2) + 0.75(1) \;=\; 1.25
\end{align*}

\vspace{1em}
\begin{center}
\highlight{Naive DiD: $1.75 - 1.25 = +0.50$ --- pure imbalance bias.}
\end{center}
\end{frame}

%% ---------- L2-DX-2a: Formal setup ----------
\begin{frame}{Formal: Y(0) trend depends on X}
\vspace{0.6em}
Let
\[
Y(0)_{it} \;=\; \alpha_i \;+\; \gamma_t \;+\; \beta_t \,X_i \;+\; \varepsilon_{it}
\]
where $X_i$ is time-invariant but its coefficient $\beta_t$ can drift.
...
%% ---------- L2-DX-2b: The product ----------
\begin{frame}{The bias factors into trend $\times$ imbalance}
\vspace{1em}
\[
\text{Bias}_{\Delta X} \;=\; \underbrace{(\beta_{\text{post}} - \beta_{\text{pre}})}_{\text{$X$-specific trend in $Y(0)$}} \;\times\; \underbrace{\big(\mathbb{E}[X \mid D{=}1] - \mathbb{E}[X \mid D{=}0]\big)}_{\text{imbalance on $X$}}
\]

\vspace{1.4em}
\begin{center}
\highlight{It's a product. \;Either factor at zero kills it.}
\end{center}
\end{frame}

%% ---------- L2-DX-2c: Either factor at zero ----------
\begin{frame}{Either factor at zero kills it}
\vspace{0.8em}
\begin{itemize}\setlength\itemsep{1.2em}
  \item Balance $X$ across groups \;$\Rightarrow$\; no bias, even with strong $\beta_t$ drift.
  \item Constant $\beta$ \;$\Rightarrow$\; no bias, even with strong imbalance.
\end{itemize}
\end{frame}

%% ---------- L2-DX-2d: Imbalance is the root ----------
\begin{frame}{Imbalance is the root}
\vspace{0.6em}
...
\vspace{0.8em}
\begin{center}
\highlight{DR / IPW / OR are just different ways to re-weight toward balance.}
\end{center}
\end{frame}

%% ---------- L2-DX-3: Bias_DeltaX — Stata, gender example ----------
\begin{frame}[fragile]{The gender example in Stata}
\footnotesize
...
\vspace{0.6em}
\begin{center}
\highlight{$1.75 - 1.25 = 0.50$. \;Imbalance $\times$ drift in $\beta_t$.}
\end{center}
\end{frame}

%% =========================================================================
%%   Title X clinics — changing-betas × imbalance, with event-study reveal
%% =========================================================================

%% ---------- L2-DX-4a: The violation we want to demonstrate ----------
\begin{frame}{A worked example of the violation}
\vspace{0.8em}
We'll build a world where:
\begin{itemize}\setlength\itemsep{0.6em}
  \item the true treatment effect is \textit{positive},
  \item the covariate's effect on $Y(0)$ \textit{changes sharply at the year of treatment},
  \item and the covariate is \textit{imbalanced} across treated and control.
\end{itemize}

\vspace{0.8em}
\begin{center}
\highlight{Then we run a plain DiD and see what we get.}
\end{center}
\end{frame}

%% ---------- L2-DX-4b: What we built in ----------
\begin{frame}{What we built in}
\vspace{0.5em}
500 counties, 10 years, treatment in year 6. \;Outcome: \;county birth rate.

\vspace{0.8em}
\begin{itemize}\setlength\itemsep{0.6em}
  \item \textbf{True effect:} \;$+0.6$ at year 6 $\to$ $+3.0$ by year 10. \;ATT $\approx +1.8$.
  \item \textbf{Covariate trend:} \;urban $Y(0)$ slope shifts from $\sim\!-0.1$ to $-2.5$ at year 6. \;Rural flat.
  \item \textbf{Imbalance:} \;$\Pr(\text{treated} \mid \text{urban}) = 0.70$, \;$\Pr(\text{treated} \mid \text{rural}) = 0.30$.
\end{itemize}

\vspace{0.8em}
\begin{center}
\highlight{Changing $\beta_t$ on urban, kicking in exactly when treatment does.}
\end{center}
\end{frame}

%% ---------- L2-DX-4c: The DGP in Stata ----------
\begin{frame}[fragile]{The DGP}
\scriptsize
...
%% ---------- L2-DX-4d: Naive event study figure ----------
\begin{frame}{Run a DiD event study}
\vspace{0.2em}
\begin{center}
\includegraphics[width=0.68\linewidth]{figures/titlex_es_naive.pdf}
\end{center}

\vspace{0.2em}
\begin{center}
\highlight{Pre-trends flat. \;Truth was $+1.8$. \;We got $-3.3$.}
\end{center}
\end{frame}

%% ---------- L2-DX-4e: We can recover this — figure ----------
\begin{frame}{We can recover this with the methods today}
\vspace{0.2em}
\begin{center}
\includegraphics[width=0.68\linewidth]{figures/titlex_es_urban.pdf}
\end{center}

\vspace{0.2em}
\begin{center}
\highlight{Same event study, conditioned on urban. \;ATT $\approx +1.75$.}
\end{center}
\end{frame}

%% ---------- L2-DX-4f: The fix codeblock (brief) ----------
\begin{frame}[fragile]{One extra variable}
\vspace{0.6em}
\begin{lstlisting}[language={[ANSI]C},basicstyle=\ttfamily\small]
* Naive
estat event after a plain DiD          // ATT ~ -3.3

* Conditioned on urban
estat event after a DiD with urban     // ATT ~ +1.75
\end{lstlisting}

\vspace{1em}
\begin{center}
\highlight{We'll meet the estimators that do this --- DR, IPW, OR --- next.}
\end{center}
\end{frame}

%% ---------- L2-DX-4g: The lesson ----------
\begin{frame}{Pre-trends don't protect us from imbalance}
\vspace{0.8em}
The pre-trends test asks: \textit{did $Y(0)$ move in parallel before treatment?}

\vspace{0.8em}
It can't see a $\beta_t$-on-$X$ shock that turns on at the year of treatment --- the one that imbalance is about to amplify.

\vspace{1em}
\begin{center}
\highlight{Pre-trends are necessary. \;They are not sufficient.}
\end{center}
...
%% ---------- L2-FE-1: Time-varying X — bad control ----------
\begin{frame}{Time-varying $X$: the bad-control problem}
\vspace{0.4em}
\textbf{The trap.} \;A time-varying covariate is measured both before \textit{and after} treatment. If $D \to X$ (treatment changes $X$), then post-period $X$ is not a pre-treatment characteristic --- it's a partial \textit{outcome}.
\vspace{0.6em}
\begin{itemize}\setlength\itemsep{0.4em}
  \item Conditioning on post-treatment $X$ blocks part of the causal channel $D \to Y$ (mediator bias).
  \item Worse, if $X$ has unobserved common causes with $Y$, conditioning opens a back-door collider path (Angrist--Pischke ``bad control'').
\end{itemize}
\vspace{0.5em}
\textbf{The condition you need.} \;$X(1) = X(0)$ --- treatment doesn't move $X$. \;Equivalently: use only pre-period $X$, since by no-anticipation $X_{i,0} = X_i(0)$ mechanically.
\vspace{0.4em}
\meta{Rule of thumb: don't condition on anything that could be a downstream consequence of $D$. \;Demographics and baseline $X$ are safe. \;Income or employment status measured post-treatment is not.}
\end{frame}

%% ---------- L2-FE-2: Time-invariant baseline X — eaten by FE ----------
\begin{frame}{Time-invariant baseline $X$: eaten by the fixed effect}
\vspace{0.4em}
\textbf{The other trap.} \;You wisely use \textit{baseline} (pre-period) $X$ to avoid the bad-control problem above. But then:
\vspace{0.6em}
\begin{itemize}\setlength\itemsep{0.4em}
  \item Within-unit demeaning makes $\tilde{X}_{it} = X_{it} - \bar{X}_i = 0$ if $X$ is time-invariant.
  \item So \texttt{race}, \texttt{education at baseline}, \texttt{pre-program earnings} --- the kind of $X$ you needed to defend CPT --- vanish.
  \item Additive-$X$ TWFE estimates as if you'd never included them.
\end{itemize}
\vspace{0.5em}
\textbf{The fix.} \;Enter $X$ as $\text{Post} \times X_{i,0}$. \;Post varies within unit, so the FE cannot eat the interaction.
\vspace{0.4em}
\meta{The double bind: time-varying $X$ is a bad control; time-invariant $X$ gets eaten by $\alpha_i$. \;$\text{Post}\times X_{i,0}$ is the escape valve.}
\end{frame}
...
\vspace{0.5em}
\meta{When you hear ``imputation'' on Day 4 (staggered DiD), you'll already know what it is. \;You've been doing it all day.}
\end{frame}
...
\vspace{0.4em}
\begin{itemize}
  \item \textbf{Day 3:} continuous treatment --- when the dose itself varies.
