...
Three steps:
\begin{enumerate}\setlength\itemsep{0.35em}\footnotesize
  ...
  \item B-spline regression of $\Delta\tilde Y_i$ on $\psi(D_i)$, \;using only treated units ($D_i > 0$). \;Here $\psi(d) = (\psi_1(d), \ldots, \psi_K(d))$ is the \textit{B-spline basis} --- $K$ pre-computed columns, one per basis function. \;In Stata: \;\texttt{mkspline} builds it.
