...
\begin{enumerate}\setlength\itemsep{0.25em}\footnotesize
  ...
  \item Bootstrap for pointwise confidence bands.
\end{enumerate}

\vspace{0.3em}
{\footnotesize\color{slate}\itshape
The ``B'' is for \textbf{basis}: $\psi(d) = (\psi_1(d), \ldots, \psi_K(d))$ is a bag of $K$ basis functions built from the knots. \;Knots set the structure; the basis is what you regress on (\texttt{mkspline} in Stata).}

\vspace{0.2em}
\begin{center}
\highlight{Returns $\widehat{\text{ATT}}(d \mid d)$ as a continuous function, with SEs at every $d$.}
\end{center}
\end{frame}
