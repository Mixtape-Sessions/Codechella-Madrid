...
\begin{frame}{Let's verify it on Kyle's data}
\vspace{0.6em}
...
%% ---------- Splines ----------
\begin{frame}{Splines: smooth piecewise polynomials}
\vspace{0.5em}

A spline is a curve made of low-degree polynomial pieces joined smoothly at \textbf{knot} points.

\vspace{0.9em}

\textbf{Cubic B-splines} (the standard):
\begin{itemize}\setlength\itemsep{0.3em}
  \item Each piece is a cubic. \;Pieces meet with continuous first and second derivatives.
  \item Place knots at sample quantiles of $D$ --- resolution where the data is.
  \item Number of knots controls smoothness: few knots = smooth, many knots = wiggly.
  \item Fit by OLS on the basis vector $\psi(D_i) = (\psi_1(D_i), \ldots, \psi_K(D_i))$.
\end{itemize}

\vspace{0.6em}
\begin{center}
\highlight{Smooth, differentiable, far fewer parameters than fine bins.}
\end{center}
\end{frame}
