...
\textbf{\color{red!70!black}Backwards engineering}
\vspace{0.2em}
\begin{itemize}\setlength\itemsep{1pt}\footnotesize
  \item Run TWFE (or OLS) of $\Delta Y_i$ on $D_i$.
  \item FWL: the coefficient is a weighted average of dose-specific treatment effects.
  \item Weights come from the variance of $D$ conditional on controls.
  \item Some weights are \textbf{negative}; they vary across the dose distribution.
  \item No recognizable causal parameter in sight.
\end{itemize}
\end{column}
...
\textbf{\color{remixgreendark}Forwards engineering}
\vspace{0.2em}
\begin{itemize}\setlength\itemsep{1pt}\footnotesize
  \item Name the target: $\text{ATT}(d \mid d)$ or $\text{ACRT}(d)$.
  \item State assumptions you are willing to defend.
  \item Build an estimator that is right on average if those assumptions hold --- and has low variance.
  \item The estimator's meaning is known \textit{before} you run it.
\end{itemize}
\end{column}
...
\vspace{0.3em}
\begin{center}
\highlight{CBS is forwards engineering. \;TWFE with continuous $D$ is backwards.}
\end{center}
\end{frame}

%% ---------- Guido on LATE ----------
\begin{frame}{Guido Imbens on this distinction}
\vspace{0.3em}

Imbens makes this point about IV and LATE (1994): \textbf{start with the parameter, then build the estimator.}

\vspace{0.5em}

\begin{itemize}\setlength\itemsep{6pt}
  \item IV with a binary instrument identifies the LATE --- the average effect for compliers.
  \item That is a \textit{well-defined} causal parameter for a \textit{well-defined} subpopulation.
  \item The alternative --- ``2SLS coefficient = something'' --- is backwards engineering. \;It works when effects are homogeneous; it obscures when they are not.
  \item The same logic applies here: with continuous $D$, commit to $\text{ATT}(d \mid d)$, then estimate it.
\end{itemize}
...
\quad --- \quad Guido Imbens on IV and LATE (MHE lecture series)}
\end{center}
\end{frame}
...
\begin{enumerate}\setlength\itemsep{0.4em}
  ...
  \item B-spline regression of $\Delta\tilde Y_i$ on $\psi(D_i)$, \;using only treated units ($D_i > 0$).
  \item Bootstrap to get pointwise confidence bands on the fitted curve.
\end{enumerate}

\vspace{0.5em}
\begin{center}
\highlight{Returns $\widehat{\text{ATT}}(d \mid d)$ as a continuous function of $d$, with standard errors at every $d$.}
\end{center}
\end{frame}
