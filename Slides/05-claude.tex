...
\begin{tabular}{lccc}
  ...
  \midrule
  Mean panel length (yrs)      & \texttt{[XX]} & \texttt{[XX]} & \texttt{[XX]} \\[3pt]
  Experiments exploited (median) & \texttt{[XX]} & \texttt{[XX]} & \texttt{[XX]} \\[3pt]
  Includes federal MW window (\%) & \texttt{[XX\%]} & \texttt{[XX\%]} & \texttt{[XX\%]} \\
  \bottomrule
\end{tabular}
\end{center}
...
If negative-arm TWFE adopters disproportionately use panels spanning 2007--2009, their estimator choice is partly \textit{data-forced}: CS and BJS cannot produce valid ATTs in that window.
The prime may direct the \textit{search}; the data structure may direct the \textit{commitment}.}
