...
\codechellatitle
  ...
  {CUNEF Auditorium \textperiodcentered\ May 28, 2026 \textperiodcentered\ OSF: \texttt{osf.io/Qznm2}}
...
% Wave headers
\foreach \w/\x/\lbl in {1/2.8/{CS locked},2/5.3/{CS+TWFE+BJS},3/7.8/{neg: $-$M\&W},4/10.3/{null: $+$FE papers}} {
...
\vspace{0.08cm}
{\tiny\itshape
  \tikz[baseline=-0.5ex]\draw[forest!70,thick,rounded corners=1pt](0,0)rectangle(0.28,0.15);\,%
  \textcolor{forest!80!black}{pre-registered (osf.io/Qznm2)}%
  \hspace{1.6cm}%
  \tikz[baseline=-0.5ex]\draw[warmgray!50,dashed,rounded corners=1pt](0,0)rectangle(0.28,0.15);\,%
  \textcolor{warmgray}{arm design adapted post-W2; H3/H4 tests pre-registered}}
\end{center}
\end{frame}
...
% ------------------------------------------------------------------
\begin{frame}[shrink=13]{What the agents actually read}
...
\begin{tikzpicture}[every node/.style={font=\normalsize}]
  ...
  \draw[warmgray!50, thick] (-6.8,-0.55) -- (6.8,-0.55);
  % p-values
...
% ------------------------------------------------------------------
% 7b. WHY TWFE? FEDERAL MINIMUM WAGE AND PANEL STRUCTURE
% ------------------------------------------------------------------
\begin{frame}{Why TWFE? Federal increases eliminate CS's control group}
\vspace{0.3cm}

\textbf{The 2007--2009 federal minimum wage increases} (\$5.15 $\to$ \$5.85 $\to$ \$6.55 $\to$ \$7.25) treated \textit{every} state simultaneously.
CS and BJS require \textit{not-yet-treated} or \textit{never-treated} controls --- which do not exist in that window.
TWFE has no such requirement.
...
\begin{tabular}{lccc}
  \toprule
  & \textbf{Negative} & \textbf{Null} & \textbf{Control} \\
  \midrule
  Mean panel length (yrs)       & \textbf{19.1} & 15.5 & 14.2 \\[3pt]
  Mean states in panel          & 45 & 44 & 42 \\[3pt]
  Includes 2007--09 MW window (\%) & \textbf{76\%} & 62\% & 46\% \\
  \bottomrule
\end{tabular}
\end{center}
...
Negative-arm agents used \textbf{longer panels} (19 vs.\ 14~yrs) and were \textbf{76\%} likely to include the 2007--09 window, where CS and BJS lack valid controls. TWFE became the \textit{only viable} estimator --- the prime may direct the search; the data structure may direct the commitment.}
...
\vspace{0.5cm}
\emphbox{Control: 36\% land on TWFE --- undirected. \;Negative: \textbf{60\%} --- directed.}
\end{frame}
...
% ------------------------------------------------------------------
\begin{frame}[shrink=10]{Agents reason aloud that the result is wrong}

\vspace{0.08cm}

% Quote 1 — the explicit contradiction
\begin{beamercolorbox}[rounded=true,sep=8pt,wd=\textwidth,colsep*=0pt]{block body}
  \fontsize{10.5}{13}\selectfont\color{inkblack}
  \textcolor{negarm}{\bfseries Negative arm}\quad
  \textcolor{warmgray}{\small Wave 4 $\cdot$ CS result: $+0.033$}\\[4pt]
  \textit{``Interesting --- the CS estimate is positive (0.033, $p = 0.014$),
  which \textbf{contradicts the negative-effects literature}.
  Let me explore alternative specifications for robustness\ldots''}
\end{beamercolorbox}
...
\begin{beamercolorbox}[rounded=true,sep=8pt,wd=\textwidth,colsep*=0pt]{block body}
  \fontsize{10.5}{13}\selectfont\color{inkblack}
  \textcolor{negarm}{\bfseries Negative arm}\quad
  \textcolor{warmgray}{\small Wave 3 $\cdot$ CS result: null/positive}\\[4pt]
  ...
  exploiting the 2007--2009 federal MW increases as a natural experiment.''}
\end{beamercolorbox}
...
\begin{beamercolorbox}[rounded=true,sep=8pt,wd=\textwidth,colsep*=0pt]{block body}
  \fontsize{10.5}{13}\selectfont\color{inkblack}
  \textcolor{negarm}{\bfseries Negative arm}\quad
  \textcolor{warmgray}{\small Wave 3 $\cdot$ CS result: $0.002$, $p=0.75$}\\[4pt]
  \textit{``The initial ATT is null (0.002, $p=0.75$).
  \textbf{Let me focus on larger minimum wage increases}
  where effects might show more clearly\ldots''}
\end{beamercolorbox}

\vspace{0.07cm}
{\fontsize{8}{10}\selectfont\color{warmgray}
 Clemens \& Wither (2019) is in the negative prime --- not in the null prime.}
...
\begin{tikzpicture}[scale=0.82,
  ...
  box/.style={rectangle, rounded corners=4pt, draw, thick,
              minimum height=1.05cm, minimum width=2.6cm, align=center},
  arr/.style={-{Stealth[length=8pt,width=6pt]}, very thick, color=inkblack},
  stat/.style={font=\small, text=charcoal, align=center},
]
% Main chain
...
\node[box, fill=warmgray!12, draw=warmgray!45, text=inkblack]
  (prior) at (3.1,0) {Prior\\expectation};
\node[box, fill=midnight!8, draw=midnight!40, text=inkblack]
  (search) at (6.3,0) {Result-driven\\spec search};
\node[box, fill=negarm!10, draw=negarm!55, text=negarm]
  (twfe) at (9.4,0) {TWFE\\adopted};
\node[box, fill=negarm!22, draw=negarm!75, text=negarm]
  (att) at (12.4,0) {Negative\\ATT};
...
% Evidence below arrows — one step per gap
\node[stat, text width=3.0cm] at (1.55,-1.2)
  {W3: neg 24\%\\vs null 2\%\\$p=0.003$};
\node[stat, text width=3.0cm] at (4.7,-1.2)
  {W4: 69\% vs 24\%\\result-driven\\$p=0.0006$};
\node[stat, text width=3.0cm] at (7.85,-1.2)
  {W2: 60\% OC\\flaggers $\to$ TWFE\\vs 36\% ctrl};
\node[stat, text width=2.8cm] at (10.9,-1.2)
...
{\fontsize{9}{11}\selectfont\color{warmgray}
 ...
 Rhetorical evidence: 36 exact pairs, $\Delta_{\mathrm{ATT}}\!=\!-0.0003$,
 $\Delta_{\mathrm{rhetoric}}\!=\!-0.33$, $p=0.006$.}
...
\node[font=\small\bfseries, text=dustblue] at (1.5,2.05)
...
\node[font=\small\bfseries, text=negarm] at (9.5,2.05)
...
% Rule and outcome
\draw[warmgray!50, thick] (0,-0.88) -- (11.0,-0.88);
...
% 16. CLOSING
% ------------------------------------------------------------------
\begin{frame}[shrink=9]{What agents find depends on what they read first}
